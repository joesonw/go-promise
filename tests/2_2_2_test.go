...
var _ = Describe("2.2.2: If `onResolved` is a function", func() {
	Describe("2.2.2.1: it must be called after `promise` is resolved, with `promise`’s resolvement value as its first argument", func() {
		testResolved("value", func(p promise.Interface, done Done) {
			p.Then(func(value interface{}) promise.Interface {
	...
	Describe("2.2.2.2: it must not be called before `promise` is resolved", func() {
		It("is resolved after a delay", func() {
		...
		It("never resolved", func() {
			...
			Expect(onResolveCalled).To(BeFalse())
		})
	})
